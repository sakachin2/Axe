//*CID://+vb94R~:                             update#=  453;       //+vb94R~
//******************************************************************************//~v003I~
//vb94:170228 (Coding:bug)stdo is duplicated with jnic2j           //+vb94I~
//vb91:170228 marge rh9j gxe to arm-gxe                            //~vb91I~
//				vb7A:170130 (gxe:Bug)accept msg even if drop failed//~vb91I~
//vak2:130822 Axe:ndk-r9 warning                                   //~vak2I~
//v@@@:110610 Android(-DARM)                                       //~v@@@I~
//va7S:100907 (BUG)mouse drag at edge of screen dose not expand copy range but scroll.//~va7SI~//~v@@@R~
...
dprintf("lbdown (y,x)=(%d,%d)-->row=%d,col=%d\n",Ppoint.y,Ppoint.x,Mcellcopy1.y,Mcellcopy1.x);//~v685R~
    mousescrollchk(Mcellcopy1.x,Mcellcopy1.y);                     //~v5a1R~
    Mcellcopy2=Mcellcopy1;                                         //~v003R~
//  rc=wxe_movecsr(Mcellcopy1.y,Mcellcopy1.x);                     //~vak2R~
       wxe_movecsr(Mcellcopy1.y,Mcellcopy1.x);                     //~vak2I~
...
dprintf("mouse move Mcpcopysw=%d,x=%d,y=%d,col=%d,row=%d\n",Mcpcopysw,Ppoint.x,Ppoint.y,Mcellcopy2.x,Mcellcopy2.y);//~v702R~//~v@@@R~
...
dprintf("mousemove Mcpcopysw=%d,posid=%x,col=%d,row=%d\n",Mcpcopysw,Mlbposid,Mcellcopy2.x,Mcellcopy2.y);//~v702R~//~v@@@R~
...
dprintf("scroll:ontimer scrolled posid=%x,Pid=%x\n",Mlbposid,Pid); //~vXXER~
			xxemain_scrinvalidate(0);                              //~vXXEI~
		    return TRUE;		//paste avail                      //~v5a1I~
        }                                                          //~v5a1I~
    }                                                              //~v5a1I~
dprintf("scroll:kill timer posid=%x,Pid=%x\n",Mlbposid,Pid);       //~vXXER~
...
//****************************                                     //~v5a1I~
    ...
    {                                                              //~v5a1I~
        ...
        {                                                          //~v51uI~
                                                                   //~v51uI~
dprintf("scroll:starttimer posid=%x,col=%d,row=%d\n",posid,Pcol,Prow);//~vXXER~
		    usettimer(posid,ms,0);                                 //~vXXER~
            Mscrolltimereventctr=0; //start timer                  //~v51uI~
        }                                                          //~v51uI~
dprintf("scroll:continueed timer posid=%x,col=%d,row=%d\n",posid,Pcol,Prow);//~vXXER~
        Mlbposid=posid;                                            //~v5a1I~
    	return 1;                                                  //~v5a1I~
    }                                                              //~v5a1I~
dprintf("scroll:set 0 posid posid=%x,col=%d,row=%d\n",posid,Pcol,Prow);//~vXXER~
#endif                                                             //~v@@@I~
    Mlbposid=0;                                                    //~v5a1I~
    return 0;                                                      //~v5a1I~
...
//    xxe_scrdisp();//display errmsg                               //~v71xR~
    ...
    {                                                              //~v76gI~
      if (!(Gxxestat & GXXES_DNDUTIL)) //not override errmsg(csr pos err)//~v76gR~
       if (Mdroprc)   //dropfile bool=success                      //~vb7AR~//~vb91I~
